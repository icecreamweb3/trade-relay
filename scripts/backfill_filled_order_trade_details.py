#!/usr/bin/env python3
"""Backfill missing trade detail fields for historical filled orders.

This script scans the orders table for FILLED rows that are missing one or more of:
- commission
- commission_asset
- realized_pnl (CLOSE orders only)

For each candidate order it queries Binance per-fill trade details and reuses the
application's existing sync logic to update the row.

Usage:
    /home/will/project/trade-relay/.venv/bin/python scripts/backfill_filled_order_trade_details.py
    /home/will/project/trade-relay/.venv/bin/python scripts/backfill_filled_order_trade_details.py --username alice
    /home/will/project/trade-relay/.venv/bin/python scripts/backfill_filled_order_trade_details.py --batch-size 200 --max-orders 500
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill missing commission / commission asset / realized pnl for filled orders.",
    )
    parser.add_argument(
        "--username",
    ...
    for batch in _iter_candidate_orders(username=args.username, batch_size=args.batch_size):
        for row in batch:
            ...
            try:
                ...
                failed += 1
                print(f"FAIL order_id={order_id} username={username} direction={trade_direction} error={exc}")
                continue
            ...
            if changed:
                ...
                skipped += 1
                print(f"SKIP order_id={order_id} username={username} direction={trade_direction} reason=no_change")
